---
layout:     post
title:      Redux 源码解析(一)  createStore.js
...
>目前所分析的`Redux`版本为`4.0.4`。`createStore`这个模块就是用于创建一个`store`对象，同时，对外暴露出`dispatch`,`getState`,`subscribe`和`replaceReducer`方法。


## 分析

`createStore`接受三个参数，我想只要你用过，你并不会陌生。
...
这个方法最终生成一个对象，对象中包含了一个重要的成员变量，即State树。也暴露了几个成员方法。
...
可以看到createStore方法最终return了几个方法，通过闭包的原理，内部的各个变量也就被持久化的存储了，而通过暴露出的getState，dispatch等函数，我们可以获取或者改变其内部的state.那么暴露出来的这几个函数都是做什么的呢？
